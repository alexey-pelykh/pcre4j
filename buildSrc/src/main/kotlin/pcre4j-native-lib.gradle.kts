...
if (project.name != "all") {
    ...
    // Registered unconditionally on every platform module. The task is wired into
    // the publish task graph below, so any publish path (Maven Local, staging deploy,
    // or direct publication task) fails fast when the bundle is empty — no workflow
    // needs to remember to invoke the verifier explicitly.
    ...
    // Wire verifyNativeBundles into the publish task graph so every publish path fails
    // fast on empty/placeholder bundles — regardless of whether the invoking workflow
    // remembers to call the verifier explicitly. Defense-in-depth at the Gradle level
    // complementing workflow-level wiring (see issue #574).
    //
    // Covers:
    //   - publishAllPublicationsToStagingDeployRepository (JReleaser staging)
    //   - publishToMavenLocal                             (local ~/.m2 and CI snapshot)
    //   - publishMavenJavaPublication*                    (per-repository publication tasks
    //                                                       generated by maven-publish)
    //
    // The :native:all aggregate module does not register verifyNativeBundles (guarded
    // by the enclosing `if (project.name != "all")`) and is therefore unaffected.
    tasks.matching { task ->
        task.name == "publishAllPublicationsToStagingDeployRepository" ||
            task.name == "publishToMavenLocal" ||
            task.name.startsWith("publishMavenJavaPublication")
    }.configureEach {
        dependsOn("verifyNativeBundles")
    }
}
